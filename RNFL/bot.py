...
async def on_ready():
    ...
    #Set bot status
    await client.change_presence(game=discord.Game(name="Week 1 starts November 4th"))
    ...
    global rFA
    global sILL
    sILL = discord.utils.find(lambda a: a.id == '503665020241051648', client.servers)
    rHC = discord.utils.find(lambda b: b.name == 'Head Coaches', sILL.roles)
    rAC = discord.utils.find(lambda c: c.name == 'Assistant Coaches', sILL.roles)
    rFO = discord.utils.find(lambda d: d.name == 'Franchise Owners', sILL.roles)
    rCO = discord.utils.find(lambda e: e.name == 'Commissioners', sILL.roles)
    ...
    global cCon
    cCon = discord.utils.find(lambda q: q.id == '503670422144024597', sILL.channels)
    print("Contract channel has been found.")
    print("Searching for promotions channel...")
    global cPro
    cPro = discord.utils.find(lambda q: q.id == '503670531242065940', sILL.channels)
...
async def on_message(message):
    if message.author.bot is False:
        ...
        if re.match(r"!\b(reset)t(mon)\b", message.content, re.IGNORECASE):
            if message.author.id == "250019187743916032" or message.author.id == "276531286443556865":
